import { getSelfDiShi, getTenGod } from '../core/ganzhi';
import type { BaziReading, ElementName } from '../core/types';
import type { YaoValue } from './types';
export { getSelfDiShi, getTenGod } from '../core/ganzhi';

export const elementIcon: Record<string, string> = {
  木: '🌱',
  火: '🔥',
  土: '⛰',
  金: '⚒',
...
export const branchElement: Record<string, string> = {
  子: '水',
  丑: '土',
  寅: '木',
  卯: '木',
...
export const elementControlledBy: Record<ElementName, ElementName> = {
  木: '金',
  火: '水',
  土: '木',
  金: '火',
...
export const seasonProfileByBranch: Record<
  string,
...
export type ElementRemedyGuide = {
  principle: string;
  timing: string[];
  environments: string[];
  workModes: string[];
...
export const elementRemedyGuide: Record<ElementName, ElementRemedyGuide> = {
  木: {
...
export const tenGodLuckMeanings: Record<string, { focus: string; opportunity: string; risk: string }> = {
...
export const palaceMeanings: Record<string, { title: string; time: string; space: string; body: string; people: string[] }> = {
...
export const kinshipByTenGod: Record<string, { family: string[]; social: string[] }> = {
...
export const stemElement: Record<string, string> = {
  甲: '木',
  乙: '木',
  丙: '火',
  丁: '火',
...
export const stemPolarity: Record<string, '阳' | '阴'> = {
  甲: '阳',
  乙: '阴',
  丙: '阳',
  丁: '阴',
...
export const branchHiddenStems: Record<string, string[]> = {
  子: ['癸'],
  丑: ['己', '癸', '辛'],
  寅: ['甲', '丙', '戊'],
  卯: ['乙'],
...
export const changShengOrder = ['长生', '沐浴', '冠带', '临官', '帝旺', '衰', '病', '死', '墓', '绝', '胎', '养'];
export const changShengStart: Record<string, string> = { 甲: '亥', 乙: '午', 丙: '寅', 丁: '酉', 戊: '寅', 己: '酉', 庚: '巳', 辛: '子', 壬: '申', 癸: '卯' };
export const monthBranches = ['寅', '卯', '辰', '巳', '午', '未', '申', '酉', '戌', '亥', '子', '丑'];
export const monthTerms = ['立春', '惊蛰', '清明', '立夏', '芒种', '小暑', '立秋', '白露', '寒露', '立冬', '大雪', '小寒'];
export const dayMs = 24 * 60 * 60 * 1000;

export const trigramByBits: Record<string, { name: string; symbol: string; nature: string; image: string }> = {
...
export const hexagramMatrix: Record<string, Record<string, { number: number; name: string }>> = {
...
export const hexagramThemes: Record<string, string> = {
...
export const divinationPresets = [
  '近期事业推进是否适合主动争取？',
  '这段关系下一步应不应该继续投入？',
  '当前合作是否值得推进？',
  '近期财运与项目回款走势如何？',
...
export const yaoPositionLabels = ['初爻', '二爻', '三爻', '四爻', '五爻', '上爻'];
export const yaoDisplayLabels = ['上爻', '五爻', '四爻', '三爻', '二爻', '初爻'];

export const tianYiMap: Record<string, string[]> = {
  甲: ['丑', '未'],
  戊: ['丑', '未'],
  庚: ['丑', '未'],
  乙: ['子', '申'],
...
export const wenChangMap: Record<string, string> = {
  甲: '巳',
  乙: '午',
  丙: '申',
  丁: '酉',
...
export const luShenMap: Record<string, string> = {
  甲: '寅',
  乙: '卯',
  丙: '巳',
  丁: '午',
...
export const yangRenMap: Record<string, string> = {
  甲: '卯',
  乙: '寅',
  丙: '午',
  丁: '巳',
...
export const taiJiMap: Record<string, string[]> = {
  甲: ['子', '午'],
  乙: ['子', '午'],
  丙: ['卯', '酉'],
  丁: ['卯', '酉'],
...
export const guoYinMap: Record<string, string> = {
  甲: '戌',
  乙: '亥',
  丙: '丑',
  丁: '寅',
...
export const fuXingMap: Record<string, string> = {
  甲: '寅',
  乙: '丑',
  丙: '子',
  丁: '亥',
...
export const tianChuMap: Record<string, string> = {
  甲: '巳',
  乙: '午',
  丙: '巳',
  丁: '午',
...
export const jinYuMap: Record<string, string> = {
  甲: '辰',
  乙: '巳',
  丙: '未',
  丁: '申',
...
export const hongYanMap: Record<string, string> = {
  甲: '午',
  乙: '申',
  丙: '寅',
  丁: '未',
...
export const hongLuanMap: Record<string, string> = {
  子: '卯',
  丑: '寅',
  寅: '丑',
  卯: '子',
...
export const tianXiMap: Record<string, string> = {
  子: '酉',
  丑: '申',
  寅: '未',
  卯: '午',
...
export const tianDeMap: Record<string, string[]> = {
  寅: ['丁'],
  卯: ['申'],
  辰: ['壬'],
  巳: ['辛'],
...
export const yueDeMap: Record<string, string[]> = {
  寅: ['丙'],
  午: ['丙'],
  戌: ['丙'],
  申: ['壬'],
...
export function collectPairNotes(values: string[], rules: string[][], fallback: string) {
...
export function advanceGanZhi(baseGanZhi: string, offset: number) {
...
export function getBranchGroupStar(referenceBranch: string, targetBranch: string, map: Record<string, string[]>, label: string) {
...
export function getElementRelation(fromElement: string, toElement: string) {
...
export function getStemBranchRelation(stem: string, branch: string) {
...
export function getPairRelations(a: string, b: string, rules: string[][]) {
  return rules.filter(([left, right]) => (left === a && right === b) || (left === b && right === a)).map(([, , label]) => label);
}

export function isYangLine(value: YaoValue) {
  return value === 7 || value === 9;
}

export function isMovingLine(value: YaoValue) {
  return value === 6 || value === 9;
}

export function changedLine(value: YaoValue): YaoValue {
  if (value === 6) {
    return 7;
  }
  if (value === 9) {
...
export function formatYao(value: YaoValue) {
  if (value === 6) {
    return '老阴';
  }
  if (value === 7) {
...
export function movingLineAdvice(index: number, value: YaoValue) {
...
export function buildDivinationDetail(base: ReturnType<typeof getHexagram>, changed: ReturnType<typeof getHexagram>, movingCount: number) {
...
export function buildDivinationReading(question: string, lines: YaoValue[]) {
  if (lines.length !== 6) {
    return null;
  }
...
export function getShenShaForBranch(reading: BaziReading, targetStem: string, targetBranch: string) {
...
export function createVirtualColumn(label: string, ganZhi: string, reading: BaziReading) {
...
export function getLuckPhase(period: BaziReading['daYun']['periods'][number]) {
  if (period.endAge <= 22) {
    return '成长期';
  }
  if (period.endAge <= 42) {
...
export function describeLuckPeriod(reading: BaziReading, period: BaziReading['daYun']['periods'][number]) {
  ...
  const action = period.isCurrent
    ...
    : period.startYear > reading.annual.year
      ? '未来进入此运前，先把专业能力、现金流和关系边界准备好，届时更容易承接机会。'
      : '这步运可作为回测样本，回看学习、迁移、事业压力、关系变化是否在此阶段明显被触发。';
  const actionSteps = [
    period.isCurrent ? '当下先做资源盘点：时间、现金流、人脉、技能各列一张清单。' : period.startYear > reading.annual.year ? '提前三年准备该运主题所需的证书、案例、作品或合作资源。' : '回看此运的关键年份，标出迁移、换岗、合作、收入和关系变化。',
...
export function buildReportText(reading: BaziReading) {
  if (reading.input.unknownHour) return [`山易排盘：${reading.input.name}`, ...reading.pillars.map(p => `${p.label} ${p.ganZhi}`), ...reading.calculation.warnings, '时辰未定，暂不输出完整详批。'].join('\n');
